-- ============================================
-- MIGRACIÓN FINAL: Sistema de autenticación
-- Sin dependencia de crypt() en runtime
-- ============================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'crypt' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) THEN
    -- Intentar instalar en schema public explícitamente
...
-- Función para login (versión con public.crypt)
...
BEGIN
  ...
  -- Verificar contraseña usando public.crypt explícitamente
...
VALUES (
  ...
  'dnzapata',
  public.crypt('Fl100190', public.gen_salt('bf', 10)),
  'David',
  'Zapata',
  'admin@nutridiab.com',
  'administrador',
